...
对输入 buffer 沿指定维度求和：`out = sum(buffer, axis=dim)`
...
| clear | 输入 | 是否在归约前初始化 out。True：归约前将 out 置零；False：在 out 已有值上累加 | 布尔 | 可选（默认 `True`） |
...
10. float16 的 clear=False 模式仅在 pto 后端支持，ascendc 后端不支持
...
src = T.alloc_ub((128,), "float16")
out = T.alloc_ub((1,), "float16")
T.reduce_sum(src, out, clear=False)
```
