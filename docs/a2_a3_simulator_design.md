...
|---|---|---|
| local `set_flag` / `wait_flag` | one latch per `(core, lane, src pipe, dst pipe, event 0..7)`; setting an outstanding latch is an error and waiting consumes it | set is ordered after the named source pipe; wait fences the named destination pipe, or the lane when the destination is `S` |
| mode-0 Cube collective | every AIC publishes one phase; every AIC wait independently advances only after all AICs have published that phase | all launched core IDs participate |
| mode-0 Vector collective | every AIV lane publishes one phase; every AIV wait independently advances only after both AIV lanes on every launched core have published that phase | all launched core IDs and both Vector lanes participate |
| mode-1 Vector collective | both AIV lanes in one group publish; each AIV wait independently advances after both publications for that phase | the two Vector lanes sharing one core/group ID participate; Cube is illegal |
| mode-2 Cube→Vector cross flag | Cube publishes one independent credit to Vector 0 and Vector 1; each vector lane consumes only its own credit | the set is ordered after its named producer pipe; the wait fences its vector lane |
...
satisfy a mode-2 Cube wait, and an unconsumed Vector 1 credit cannot satisfy another Vector 0
wait. Modes 0 and 1 retain per-waiter phase counters: one participant waiting does not consume
