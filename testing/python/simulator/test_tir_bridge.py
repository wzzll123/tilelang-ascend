...
@T.prim_func
def flattened_allocate_copy(
    source: T.Buffer((2, 8), "float32"),
    output: T.Buffer((2, 8), "float32"),
):
    ub = T.allocate([16], "float32", "shared.ub")
    T.call_extern(
        "handle",
        "tl::ascend::copy_gm_to_ub<float, 8, 2>",
        source.access_ptr("r"),
        T.tvm_access_ptr(T.type_annotation("float32"), ub, 0, 16, 2),
        8,
        2,
        8,
        T.float32(0),
    ...
    T.call_extern(
        "handle",
        "tl::ascend::copy_ub_to_gm<float, 8, 2>",
        T.tvm_access_ptr(T.type_annotation("float32"), ub, 0, 16, 1),
        output.access_ptr("w"),
        8,
        2,
        8,
...
def _sync_calls_primfunc(*calls):
    statements = [tvm.tir.Evaluate(call) for call in calls]
    body = statements[0] if len(statements) == 1 else tvm.tir.SeqStmt(statements)
    return tvm.tir.PrimFunc([], body)


def _resource_scope_primfunc(scope: int, *calls):
    body = tvm.tir.SeqStmt([tvm.tir.Evaluate(call) for call in calls])
    scoped = tvm.tir.AttrStmt(
        tvm.tir.IntImm("int32", scope), "resource_scope", scope, body
    )
    return tvm.tir.PrimFunc([], scoped)


def _threaded_resource_scope_primfunc(
    thread_tag: str, extent: int, scope: int, *calls
):
    thread_var = tvm.tir.Var("thread_index", "int32")
    iteration = tvm.tir.IterVar(
    ...
    threaded = tvm.tir.AttrStmt(iteration, "thread_extent", extent, scoped)
    return tvm.tir.PrimFunc([], threaded)


def test_manual_sync_intrinsics_preserve_pipe_flag_and_mode_metadata() -> None:
    program = build_kernel_program(
        _sync_calls_primfunc(
            tvm.tir.call_extern(
                "handle", "set_flag", "PIPE_MTE2", "PIPE_V", 4
            ),
            tvm.tir.call_extern(
                "handle", "set_cross_flag", "PIPE_FIX", 8, 2
            ),
            tvm.tir.call_extern(
                "handle", "wait_cross_flag", 8, "PIPE_V"
            ),
            tvm.tir.call_extern("handle", "pipe_barrier", "PIPE_ALL"),
        ),
        platform="A2",
    )

    tasks = program.cores[0].tasks
    assert tasks[0].metadata["src_pipe"] == "mte2"
    assert tasks[0].metadata["dst_pipe"] == "v"
    assert tasks[0].metadata["flag_id"] == 4
    assert tasks[0].pipe is Pipe.SCALAR
    assert tasks[1].metadata["src_pipe"] == "fix"
    assert tasks[1].metadata["flag_id"] == 8
    assert tasks[1].metadata["mode"] == 2
    assert tasks[1].pipe is Pipe.SCALAR
    assert tasks[2].metadata["flag_id"] == 8
    assert tasks[2].metadata["wait_pipe"] == "v"
    assert tasks[2].pipe is Pipe.SCALAR
    assert tasks[3].metadata["target_pipe"] == "all"
...
def test_auto_sync_intrinsics_decode_event_pair_and_cross_arguments() -> None:
    program = build_kernel_program(
        _sync_calls_primfunc(
            tvm.tir.call_extern("handle", "auto_set_flag", "MTE2_V", 3),
            tvm.tir.call_extern("handle", "auto_wait_flag", "MTE2_V", 3),
            tvm.tir.call_extern(
                "handle", "auto_set_cross_flag", 2, "PIPE_FIX", 8
            ),
            tvm.tir.call_extern(
                "handle", "auto_wait_cross_flag", 8, "PIPE_V"
            ),
            tvm.tir.call_extern("handle", "auto_barrier", "PIPE_ALL"),
        ),
        platform="A3",
    )
    ...
    tasks = program.cores[0].tasks
    for task in tasks[:2]:
        assert task.metadata["src_pipe"] == "mte2"
        assert task.metadata["dst_pipe"] == "v"
        assert task.metadata["flag_id"] == 3
    assert tasks[0].pipe is Pipe.SCALAR
    assert tasks[1].pipe is Pipe.SCALAR
    assert tasks[2].metadata["mode"] == 2
    assert tasks[2].metadata["src_pipe"] == "fix"
    assert tasks[2].metadata["flag_id"] == 8
    assert tasks[2].pipe is Pipe.SCALAR
    assert tasks[3].metadata["flag_id"] == 8
    assert tasks[3].metadata["wait_pipe"] == "v"
    assert tasks[3].pipe is Pipe.SCALAR
    assert tasks[4].metadata["target_pipe"] == "all"
    assert tasks[4].pipe is Pipe.SCALAR


@pytest.mark.parametrize("mode", [0, 1, 2])
@pytest.mark.parametrize("operation", ["set_cross_flag", "auto_set_cross_flag"])
def test_tir_bridge_preserves_every_cross_flag_mode(
    mode: int, operation: str
) -> None:
    arguments = (
        ("PIPE_V", 6, mode)
        if operation == "set_cross_flag"
        else (mode, "PIPE_V", 6)
    )
    program = build_kernel_program(
        _sync_calls_primfunc(
            tvm.tir.call_extern("handle", operation, *arguments)
        ),
        platform="A2",
    )

    task = program.cores[0].tasks[0]
    assert task.metadata["src_pipe"] == "v"
    assert task.metadata["flag_id"] == 6
    assert task.metadata["mode"] == mode


def test_sync_markers_execute_on_named_pipes_inside_resource_scopes() -> None:
    cube = build_kernel_program(
        _resource_scope_primfunc(
            0,
            tvm.tir.call_extern(
                "handle", "set_cross_flag", "PIPE_FIX", 2, 2
            ),
            tvm.tir.call_extern("handle", "wait_cross_flag", 3, ""),
        ),
        platform="A2",
    )
    vector = build_kernel_program(
        _resource_scope_primfunc(
            ...
            tvm.tir.call_extern(
                "handle", "wait_flag", "PIPE_MTE2", "PIPE_V", 4
            ),
        ),
        platform="A3",
    )

    assert [task.lane for task in cube.cores[0].tasks] == [Lane.CUBE, Lane.CUBE]
    assert [task.pipe for task in cube.cores[0].tasks] == [Pipe.FIX, Pipe.SCALAR]
    assert [task.lane for task in vector.cores[0].tasks] == [
        Lane.VECTOR_0, Lane.VECTOR_0,
    ]
    assert [task.pipe for task in vector.cores[0].tasks] == [Pipe.MTE2, Pipe.VECTOR]


def test_mode_zero_collective_runs_from_multicore_tir() -> None:
    program = build_kernel_program(
        _threaded_resource_scope_primfunc(
            "blockIdx.x",
            2,
            0,
            tvm.tir.call_extern(
                "handle", "set_cross_flag", "PIPE_FIX", 5, 0
            ),
            tvm.tir.call_extern("handle", "wait_cross_flag", 5, ""),
        ),
        platform="A2",
    )
    ...
    assert len(program.cores) == 2
    assert sum(record.operation == "set_cross_flag" for record in result.records) == 2
    assert sum(record.operation == "wait_cross_flag" for record in result.records) == 2


def test_mode_one_collective_runs_from_two_vector_lane_tir() -> None:
    program = build_kernel_program(
        _threaded_resource_scope_primfunc(
            "blockIdx.y",
            2,
            1,
            tvm.tir.call_extern(
                "handle", "auto_set_cross_flag", 1, "PIPE_V", 4
            ),
            tvm.tir.call_extern(
                "handle", "auto_wait_cross_flag", 4, ""
            ),
        ),
        platform="A3",
    )
    ...
    assert {task.lane for task in program.tasks} == {Lane.VECTOR_0, Lane.VECTOR_1}
    assert sum(
        record.operation == "auto_set_cross_flag" for record in result.records
    ) == 2
    assert sum(
        record.operation == "auto_wait_cross_flag" for record in result.records
    ) == 2
...
def _gemm_v0_loop_init_primfunc():
    prim_func, _, _ = _gemm_v0_primfunc(inner=16)
    root = prim_func.body.block
    original = root.body.value
    loop_var = tvm.tir.Var("ko", "int32")
    loop_call = tvm.tir.Call(
        original.dtype,
        original.op,
        [*original.args[:5], loop_var == 0, original.args[6]],
    )
    loop = tvm.tir.For(
        loop_var, 0, 2, tvm.tir.ForKind.SERIAL, tvm.tir.Evaluate(loop_call)
    )
    loop_root = tvm.tir.Block(
        [], [], [], "root", loop, alloc_buffers=root.alloc_buffers
    )
    return tvm.tir.PrimFunc([], tvm.tir.BlockRealize([], True, loop_root))
...
def _reinterpretcast_primfunc(
    *, destination_extent=8, cast_type="uint16_t", planned=False
):
    ...
    function = function.with_attr("address_map", {
        ub_source.data: tvm.tir.IntImm("int64", 0),
        ub_view.data: tvm.tir.IntImm("int64", 32),
    })
    return function.with_attr("size_map", {
        ub_source.data: tvm.tir.IntImm("int64", 16),
        ub_view.data: tvm.tir.IntImm("int64", 16),
    })
...
def _topk_primfunc(
    *, dtype="float32", k=4, actual_num=7, max_actual_num=9,
    ...
    scratch_extent = scratch_extent or aligned_count * (10 if dtype == "float16" else 4)
    scratch = tvm.tir.decl_buffer(
        (scratch_extent,), dtype, name="scratch", scope="shared.ub"
    )
...
def _sort_primfunc(
    *, dtype="float32", actual_num=37, capacity=64, repeat_times=None,
    destination_extent=None, template_dtype=None, scratch_extent=None,
):
    repeat_times = (
    ...
    scratch_extent = scratch_extent or capacity * (8 if dtype == "float16" else 2)
    scratch = tvm.tir.decl_buffer(
        (scratch_extent,), dtype, name="scratch", scope="shared.ub"
    )
    dtype_name = template_dtype or {"float16": "half", "float32": "float"}[dtype]
...
def _merge_sort_primfunc(
    ...
    dtype="float32", reported_block_length=None, source_offset=0,
    destination_offset=0, scratch_extent=None, scratch_offset=0,
):
    ...
    scratch_extent = scratch_extent or num_ways * source_extent * 4
    sources = [
        tvm.tir.decl_buffer(
    ...
    ub_sources = [
        tvm.tir.decl_buffer(
            (source_offset + source_extent,), dtype, name=f"ub_source{index}",
            scope="shared.ub",
        )
        for index in range(num_ways)
    ]
    ub_output = tvm.tir.decl_buffer(
        (destination_offset + destination_extent,), dtype, name="ub_output",
        scope="shared.ub",
    )
    scratch = tvm.tir.decl_buffer(
        (scratch_offset + scratch_extent,), "uint8", name="scratch",
        scope="shared.ub",
    )
    ...
    arguments = [
        f"MergeSort<{dtype_name}>", num_ways,
        ub_output.access_ptr("w", offset=destination_offset),
    ]
    if with_scratch:
        arguments.append(scratch.access_ptr(
            "w", offset=scratch_offset, extent=scratch_extent,
        ))
    arguments.extend(
        source.access_ptr("r", offset=source_offset)
    ...
    statements = [
        tvm.tir.Evaluate(tvm.tir.call_extern(
            ...
            ub_source.access_ptr("w", offset=source_offset), source_extent,
        ))
    ...
    statements.extend([
        ...
        tvm.tir.Evaluate(tvm.tir.call_extern(
            "handle", "copy_ub_to_gm",
            ub_output.access_ptr("r", offset=destination_offset),
            output.access_ptr("w"), num_ways * source_extent,
        )),
    ])
...
def test_flattened_allocate_restores_copy_physical_row_stride() -> None:
    program = build_kernel_program(flattened_allocate_copy, platform="A2")
    load, store = program.tasks
    assert load.metadata["dst"].buffer == "ub"
    assert load.metadata["dst"].strides_bytes == (32, 4)
    assert store.metadata["src"].buffer == "ub"
    assert store.metadata["src"].strides_bytes == (32, 4)
    ...
    np.testing.assert_array_equal(simulator.read(store.metadata["dst"]), values)
...
def _exp_experiment_primfunc(
    *, dtype="float32", rows=3, physical_cols=None, column=None,
    extent=None, template_dtype=None,
):
    itemsize = np.dtype(dtype).itemsize
    chunk = 128 if dtype == "float16" else 64
    physical_cols = physical_cols or chunk * 4
    column = chunk if column is None else column
    extent = rows * chunk if extent is None else extent
    template_dtype = template_dtype or (
        "half" if dtype == "float16" else "float"
    )
    source = tvm.tir.decl_buffer(
        (rows, physical_cols), dtype, name="source", scope="global"
    )
    output = tvm.tir.decl_buffer(
        (rows, physical_cols), dtype, name="output", scope="global"
    )
    ub = tvm.tir.decl_buffer(
        (rows, physical_cols), dtype, name="exp_ub", scope="shared.ub"
    )
    count = rows * physical_cols
    ...
    body = tvm.tir.SeqStmt([
        copy("copy_gm_to_ub", source, ub),
        tvm.tir.Evaluate(tvm.tir.call_extern(
            "handle", "tl.ascend_exp_experiment",
            f"exp_mask<{template_dtype}>",
            ub.access_ptr("w", offset=column, extent=extent),
            ub.access_ptr("r", offset=column, extent=extent),
        )),
        copy("copy_ub_to_gm", ub, output),
    ])
    root = tvm.tir.Block(
        [], [], [], "root", body, alloc_buffers=[ub]
    )
    return tvm.tir.PrimFunc(
        [source.data, output.data],
        tvm.tir.BlockRealize([], True, root),
...
def test_exp_experiment_executes_strided_2d_window(platform, dtype) -> None:
    program = build_kernel_program(
        _exp_experiment_primfunc(dtype=dtype), platform=platform
    )
    load, exp_task, store = program.tasks
    chunk = 128 if dtype == "float16" else 64
    physical_cols = chunk * 4
    assert exp_task.metadata["exp_experiment"] == {
        "rows": 3,
        "chunk": chunk,
        "physical_cols": physical_cols,
    }
    assert exp_task.dependencies == (load.task_id,)
    assert store.dependencies == (exp_task.task_id,)

    values = np.linspace(
        -2, 1, 3 * physical_cols, dtype=np.dtype(dtype)
    ).reshape(3, physical_cols)
    expected = values.copy()
    expected[:, chunk:2 * chunk] = np.exp(
        expected[:, chunk:2 * chunk]
    ).astype(dtype)
    simulator = FunctionalSimulator(program)
    simulator.write(load.metadata["src"], values.reshape(-1))
    simulator.run()
    np.testing.assert_allclose(
        simulator.read(store.metadata["dst"]).reshape(3, physical_cols),
        expected,
        rtol=1e-3,
        atol=1e-3,
    )
...
def test_exp_experiment_validates_extent_stride_alignment_and_dtype() -> None:
    with pytest.raises(ProgramValidationError, match="whole 64-element rows"):
        build_kernel_program(
            _exp_experiment_primfunc(extent=65), platform="A2"
        )
    with pytest.raises(ProgramValidationError, match="32-byte aligned"):
        build_kernel_program(
            _exp_experiment_primfunc(column=1), platform="A2"
        )
    with pytest.raises(ProgramValidationError, match="crosses a physical row"):
        build_kernel_program(
            _exp_experiment_primfunc(column=224), platform="A3"
        )
    with pytest.raises(ProgramValidationError, match="tag dtype"):
        build_kernel_program(
            _exp_experiment_primfunc(template_dtype="half"), platform="A3"
        )
...
def test_gemm_v0_resolves_loop_dependent_init_flag_per_iteration() -> None:
    program = build_kernel_program(_gemm_v0_loop_init_primfunc(), platform="A3")
    assert [task.metadata["gemm"]["init"] for task in program.tasks] == [True, False]
    assert "accumulator" not in program.tasks[0].metadata
    assert program.tasks[1].metadata["accumulator"] == program.tasks[1].metadata["dst"]
...
def test_reinterpretcast_materializes_storage_rewritten_view() -> None:
    program = build_kernel_program(
        _reinterpretcast_primfunc(planned=True), platform="A2"
    )
    load, reinterpret, view_store, _, after_store = program.tasks
    assert reinterpret.metadata["materialize_view"] is True
    values = np.array(
        [0x11223344, 0x55667788, 0x99AABBCC, 0xDDEEFF00], dtype=np.uint32
    )
    simulator = FunctionalSimulator(program)
    ...
    np.testing.assert_array_equal(simulator.read(after_store.metadata["dst"]), values)
...
def test_topk_validates_static_contract() -> None:
    ...
    with pytest.raises(ProgramValidationError, match="scratch extent is too small"):
        build_kernel_program(
            _topk_primfunc(scratch_extent=127), platform="A2"
        )
...
def test_sort_validates_repeat_extent_and_template() -> None:
    ...
    with pytest.raises(ProgramValidationError, match="scratch extent is too small"):
        build_kernel_program(
            _sort_primfunc(scratch_extent=127), platform="A2"
        )
...
@pytest.mark.parametrize("platform", ["A2", "A3"])
def test_merge_sort_executes_aligned_source_and_destination_offsets(platform) -> None:
    program = build_kernel_program(
        _merge_sort_primfunc(source_offset=8, destination_offset=8),
        platform=platform,
    )
    first, second, merge, store = program.tasks
    assert all(region.byte_offset == 32 for region in merge.metadata["src_regions"])
    assert merge.metadata["dst"].byte_offset == 32

    simulator = FunctionalSimulator(program)
    first_pairs = np.array([9, 90, 7, 70, 5, 50, 3, 30], dtype=np.float32)
    second_pairs = np.array([8, 80, 6, 60, 4, 40, 2, 20], dtype=np.float32)
    simulator.write(first.metadata["src"], first_pairs)
    simulator.write(second.metadata["src"], second_pairs)
    simulator.run()
    expected = np.array(
...
def test_merge_sort_rejects_unaligned_offsets_and_small_scratch() -> None:
    with pytest.raises(ProgramValidationError, match="source0 pointer.*aligned"):
        build_kernel_program(
            _merge_sort_primfunc(source_offset=1), platform="A2"
        )
    with pytest.raises(ProgramValidationError, match="destination pointer.*aligned"):
        build_kernel_program(
            _merge_sort_primfunc(destination_offset=1), platform="A2"
        )
    with pytest.raises(ProgramValidationError, match="scratch pointer.*aligned"):
        build_kernel_program(
            _merge_sort_primfunc(with_scratch=True, scratch_offset=1),
            platform="A3",
        )
    with pytest.raises(ProgramValidationError, match="scratch extent is too small"):
        build_kernel_program(
            _merge_sort_primfunc(with_scratch=True, scratch_extent=63),
            platform="A3",
        )


def test_merge_sort_rejects_nan_until_a2_a3_ordering_is_confirmed() -> None:
    program = build_kernel_program(_merge_sort_primfunc(), platform="A2")
    first, second, _, _ = program.tasks
    simulator = FunctionalSimulator(program)
    simulator.write(
        first.metadata["src"],
        np.array([np.nan, 0, 3, 1, 2, 2, 1, 3], dtype=np.float32),
    )
    simulator.write(
        second.metadata["src"],
        np.array([4, 4, 3, 5, 2, 6, 1, 7], dtype=np.float32),
    )
    with pytest.raises(ProgramValidationError, match="NaN ordering"):
        simulator.run()
...
def test_flattened_reduce_uses_abi_row_stride() -> None:
    source = tvm.tir.decl_buffer(
        (32 * 64,), "float32", name="flat_reduce_source", scope="shared.ub"
    )
    destination = tvm.tir.decl_buffer(
        (32,), "float32", name="flat_reduce_destination", scope="shared.ub"
    )
    scratch = tvm.tir.decl_buffer(
        (8192,), "uint8", name="flat_reduce_scratch", scope="shared.ub"
    )
    body = tvm.tir.Evaluate(tvm.tir.call_extern(
        "handle",
        "tl.ascend_reduce",
        "reduce_max<float, 32, 64, -1>",
        destination.access_ptr("w"),
        source.access_ptr("r"),
        scratch.access_ptr("w"),
        True,
    ))
    root = tvm.tir.Block(
        [], [], [], "root", body, alloc_buffers=[source, destination, scratch]
    )
    program = build_kernel_program(
        tvm.tir.PrimFunc([], tvm.tir.BlockRealize([], True, root)), platform="A2"
    )

    (reduce,) = program.tasks
    assert reduce.metadata["src"].shape == (32, 64)
    assert reduce.metadata["src"].strides_bytes == (64 * 4, 4)


def test_indexed_scalar_vector_form_reads_selected_ub_element() -> None:
    source = tvm.tir.decl_buffer((4,), "float32", name="indexed_source", scope="global")
    scalars = tvm.tir.decl_buffer((2,), "float32", name="indexed_scalars", scope="global")
    output = tvm.tir.decl_buffer((4,), "float32", name="indexed_output", scope="global")
    ub_source = tvm.tir.decl_buffer((4,), "float32", name="indexed_source_ub", scope="shared.ub")
    ub_scalars = tvm.tir.decl_buffer((2,), "float32", name="indexed_scalars_ub", scope="shared.ub")
    ub_output = tvm.tir.decl_buffer((4,), "float32", name="indexed_output_ub", scope="shared.ub")

    def copy(name, src, dst, count):
        return tvm.tir.Evaluate(tvm.tir.call_extern(
            "handle", name, src.access_ptr("r"), dst.access_ptr("w"), count
        ))
    ...
    body = tvm.tir.SeqStmt([
        ...
        copy("copy_gm_to_ub", scalars, ub_scalars, 2),
        tvm.tir.Evaluate(tvm.tir.call_extern(
            "handle", "tl.ascend_subs", ub_output.access_ptr("w"),
            ub_source.access_ptr("r"), ub_scalars.access_ptr("r"), 1, 4,
        )),
        copy("copy_ub_to_gm", ub_output, output, 4),
    ])
    root = tvm.tir.Block(
        [], [], [], "root", body,
        alloc_buffers=[ub_source, ub_scalars, ub_output],
    )
    primfunc = tvm.tir.PrimFunc(
        [source.data, scalars.data, output.data],
        tvm.tir.BlockRealize([], True, root),
        buffer_map={source.data: source, scalars.data: scalars, output.data: output},
    )
    program = build_kernel_program(primfunc, platform="A3")
    source_load, scalar_load, subtract, store = program.tasks
    simulator = FunctionalSimulator(program)
    values = np.arange(4, dtype=np.float32)
    scalar_values = np.array([10.0, 1.5], dtype=np.float32)
    simulator.write(source_load.metadata["src"], values)
    simulator.write(scalar_load.metadata["src"], scalar_values)

    simulator.run()

    assert subtract.metadata["scalar_src"].byte_offset == 4
    np.testing.assert_array_equal(
        simulator.read(store.metadata["dst"]), values - scalar_values[1]
    )
...
def _im2col_primfunc(
    *, image_shape=(5, 7), kernel=(3, 3), stride=(1, 1),
    ...
    source = tvm.tir.decl_buffer(
        (source_elements,), dtype, name="im2col_source", scope="shared.l1"
    )
    destination = tvm.tir.decl_buffer(
    ...
    root = tvm.tir.Block(
        [], [], [], "root", tvm.tir.Evaluate(call),
        alloc_buffers=[source, destination],
    )
    return (
        tvm.tir.PrimFunc([], tvm.tir.BlockRealize([], True, root)),
        (output_h, output_w, valid_m, valid_k),
    )
...
def test_im2col_extracts_padded_strided_dilated_l0a_tile(
    ...
    primfunc, (output_h, output_w, valid_m, valid_k) = _im2col_primfunc(
        **configuration
    )
    program = build_kernel_program(primfunc, platform=platform)
    (task,) = program.tasks
    assert task.operation == "im2col"
    assert task.lane is Lane.CUBE
    assert task.pipe is Pipe.MTE1

    hi, wi = configuration["image_shape"]
    kh, kw = configuration["kernel"]
    ...
    for m in range(valid_m):
        oh, ow = divmod(m, output_w)
        for filter_row in range(kh):
            for filter_col in range(kw):
                image_row = oh * sh - pt + filter_row * dh
                image_col = ow * sw - pl + filter_col * dw
                if 0 <= image_row < hi and 0 <= image_col < wi:
                    start = (filter_row * kw + filter_col) * channels
                    expected[m, start:start + channels] = feature[
                        image_row * wi + image_col
                    ]

    simulator = FunctionalSimulator(program)
    simulator.write(task.metadata["src"], pack_matrix(feature, "zn"))
    simulator.run()
    physical = simulator.read(task.metadata["dst"])
    np.testing.assert_array_equal(
        unpack_matrix(physical, "l0a", (valid_m, valid_k)), expected
    )
    assert task.metadata["im2col"]["output_shape"] == (output_h, output_w)


def test_im2col_rejects_nonzero_hardware_start_point() -> None:
