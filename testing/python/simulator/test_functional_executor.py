...
def test_two_vector_lanes_use_independent_local_memory_banks() -> None:
    ub = _region("ub", MemoryScope.UB, 1)
    output_0 = _region("out", MemoryScope.GM, 1)
    output_1 = _region("out", MemoryScope.GM, 1, offset=1)
    program = KernelProgram(
        "vector-lane-local-memory",
        "A2",
        (CoreProgram(0, (
            Task(
                "fill-v0", "fill", 0, Lane.VECTOR_0, Pipe.VECTOR, 1,
                metadata={"dst": ub, "scalar": 1.0},
            ),
            Task(
                "store-v0", "copy_ub_to_gm", 0, Lane.VECTOR_0, Pipe.MTE3, 1,
                dependencies=("fill-v0",), metadata={"src": ub, "dst": output_0},
            ),
            Task(
                "fill-v1", "fill", 0, Lane.VECTOR_1, Pipe.VECTOR, 1,
                metadata={"dst": ub, "scalar": 2.0},
            ),
            Task(
                "store-v1", "copy_ub_to_gm", 0, Lane.VECTOR_1, Pipe.MTE3, 1,
                dependencies=("fill-v1",), metadata={"src": ub, "dst": output_1},
            ),
        )),),
        buffers=(
            BufferSpec("ub", MemoryScope.UB, (1,), "float32"),
            BufferSpec("out", MemoryScope.GM, (2,), "float32"),
        ),
    )
    simulator = FunctionalSimulator(program)
    ...
    np.testing.assert_array_equal(
        simulator.read(_region("out", MemoryScope.GM, 2)),
