...
当前基线为 `576 passed`。TVM 在 Python 3.13 下会产生 parser deprecation warnings；这些
...
- [x] ~~从 execution record 的首次/末次实际访问推导 allocation live interval，按
  owner/scope 的物理地址 union 去重 alias，并输出随 cycle 变化的 local-memory counter。~~
