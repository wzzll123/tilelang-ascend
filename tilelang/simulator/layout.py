...
def physical_index(
    ...
    zN/nZ follow ``tilelang/intrinsics/ascend_layout.py``. A2 L0A zZ and L0B
    nZ follow the Catlass roles in ``tl_templates/ascend/common.h``. L0C
    follows ``tla::MakeLayoutL0C`` and EasyASC's fixed-C0 NZ decoder.
    """
    rows, cols, elements_per_c0 = _validate(shape, itemsize)
